...
void vftr_MPI_Iallgather_f082vftr(void *sendbuf, MPI_Fint *sendcount,
   ...
   if (isintercom) {
      c_error = vftr_MPI_Iallgather_intercom(sendbuf,
                                             (int)(*sendcount),
                                             c_sendtype,
                                             recvbuf,
                                             (int)(*recvcount),
      ...
      if (vftr_is_C_MPI_IN_PLACE(sendbuf)) {
         c_error = vftr_MPI_Iallgather_inplace(sendbuf,
                                               (int)(*sendcount),
                                               c_sendtype,
                                               recvbuf,
                                               (int)(*recvcount),
         ...
         c_error = vftr_MPI_Iallgather(sendbuf,
                                       (int)(*sendcount),
                                       c_sendtype,
                                       recvbuf,
                                       (int)(*recvcount),
