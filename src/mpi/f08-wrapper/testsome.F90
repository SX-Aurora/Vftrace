...
SUBROUTINE MPI_Testsome_f08(incount, array_of_requests, outcount, &
   ...
   USE vftr_mpi_testsome_f082vftr_f08i, &
      ONLY : vftr_MPI_Testsome_f082vftr
   ...
   IF (vftr_no_mpi_logging_f08()) THEN
      CALL PMPI_Testsome(incount, array_of_requests, outcount, &
                         array_of_indices, array_of_statuses, tmperror)
   ELSE
      DO i = 1, incount
         tmparray_of_requests(i) = array_of_requests(i)%MPI_VAL
      END DO
      CALL vftr_MPI_Testsome_f082vftr(incount, tmparray_of_requests, outcount, &
                                      array_of_indices, array_of_statuses, tmperror)
      DO i = 1, incount
