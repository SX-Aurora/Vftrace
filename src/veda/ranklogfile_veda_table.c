...
int *vftr_ranklogfile_veda_table_ncalls_list(int nstacks, stack_t **stack_ptrs) {
   int *ncalls_list = (int*) malloc(nstacks*sizeof(int));
   for (int istack=0; istack<nstacks; istack++) {
      stack_t *stack_ptr = stack_ptrs[istack];
      ncalls_list[istack] = 0;
      for (int iprof=0; iprof<stack_ptr->profiling.nprofiles; iprof++) {
         profile_t *prof_ptr = stack_ptr->profiling.profiles+iprof;
         ncalls_list[istack] += prof_ptr->vedaprof.ncalls;
      }
   }
   return ncalls_list;
}

double *vftr_ranklogfile_veda_table_average_memcpy_HtoD_list(
   int nstacks, stack_t **stack_ptrs) {
   ...
   for (int istack=0; istack<nstacks; istack++) {
      ...
      for (int iprof=0; iprof<stack_ptr->profiling.nprofiles; iprof++) {
         profile_t *prof_ptr = stack_ptr->profiling.profiles+iprof;
         memcpy_HtoD_list[istack] += prof_ptr->vedaprof.HtoD_bytes;
...
double *vftr_ranklogfile_veda_table_average_memcpy_DtoH_list(
   int nstacks, stack_t **stack_ptrs) {
   ...
   for (int istack=0; istack<nstacks; istack++) {
      ...
      for (int iprof=0; iprof<stack_ptr->profiling.nprofiles; iprof++) {
         profile_t *prof_ptr = stack_ptr->profiling.profiles+iprof;
         memcpy_DtoH_list[istack] += prof_ptr->vedaprof.DtoH_bytes;
         ncalls += prof_ptr->vedaprof.ncalls;
      }
      if (ncalls > 0) {
...
double *vftr_ranklogfile_veda_table_average_memcpy_HtoD_bw_list(
   int nstacks, stack_t **stack_ptrs) {
...
double *vftr_ranklogfile_veda_table_average_memcpy_DtoH_bw_list(
   int nstacks, stack_t **stack_ptrs) {
...
double *vftr_ranklogfile_veda_table_avg_time_list(int nstacks, stack_t **stack_ptrs) {
...
char **vftr_ranklogfile_veda_table_stack_function_name_list(int nstacks, stack_t **stack_ptrs) {
...
char **vftr_ranklogfile_veda_table_stack_caller_name_list(int nstacks,
...
char **vftr_ranklogfile_veda_table_callpath_list(int nstacks,
                                            stack_t **stack_ptrs,
                                            stacktree_t stacktree) {
...
int *vftr_ranklogfile_veda_table_stack_globalstackID_list(int nstacks,
                                                     stack_t **stack_ptrs) {
...
int vftr_ranklogfile_veda_table_nrows(stacktree_t stacktree) {
...
stack_t **vftr_ranklogfile_veda_table_get_relevant_stacks(
   int nrows, stacktree_t stacktree) {
...
void vftr_write_ranklogfile_veda_table(FILE *fp, stacktree_t stacktree,
                                   config_t config) {
   SELF_PROFILE_START_FUNCTION;
   int nrows = vftr_ranklogfile_veda_table_nrows(stacktree);
   stack_t **selected_stacks =
      vftr_ranklogfile_veda_table_get_relevant_stacks(nrows, stacktree);
   //TODO vftr_sort_stacks_for_vedaprof(config, nrows, selected_stacks);

   fprintf(fp, "\nVEDA profile\n");
   table_t table = vftr_new_table();
   vftr_table_set_nrows(&table, nrows);

   int *ncalls = vftr_ranklogfile_veda_table_ncalls_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_int, "ncalls", "%d", 'c', 'r', (void*) ncalls);

   double *avg_memcpy_HtoD_list = vftr_ranklogfile_veda_table_average_memcpy_HtoD_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_double, "avg cpy H2D[B]", "%.3e", 'c', 'r', (void*) avg_memcpy_HtoD_list);
   
   double *avg_memcpy_DtoH_list = vftr_ranklogfile_veda_table_average_memcpy_DtoH_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_double, "avg cpy D2H[B]", "%.3e", 'c', 'r', (void*) avg_memcpy_DtoH_list);

   double *avg_memcpy_HtoD_bw_list = vftr_ranklogfile_veda_table_average_memcpy_HtoD_bw_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_double, "avg cpy H2D BW[B/s]", "%.3e", 'c', 'r', (void*) avg_memcpy_HtoD_bw_list);
   
   double *avg_memcpy_DtoH_bw_list = vftr_ranklogfile_veda_table_average_memcpy_DtoH_bw_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_double, "avg cpy D2H BW[B/s]", "%.3e", 'c', 'r', (void*) avg_memcpy_DtoH_bw_list);

   double *avg_time_list = vftr_ranklogfile_veda_table_avg_time_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_double, "avg time[s]", "%.3e", 'c', 'r', (void*) avg_time_list);

   char **function_names = vftr_ranklogfile_veda_table_stack_function_name_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_string, "Function", "%s", 'c', 'r', (void*) function_names);

   char **caller_names = vftr_ranklogfile_veda_table_stack_caller_name_list(nrows, stacktree, selected_stacks);
   vftr_table_add_column(&table, col_string, "Caller", "%s", 'c', 'r', (void*) caller_names);

   int *stack_IDs = vftr_ranklogfile_veda_table_stack_globalstackID_list(nrows, selected_stacks);
   vftr_table_add_column(&table, col_int, "STID", "%d", 'c', 'r', (void*) stack_IDs);

   char **path_list=NULL;
//   if (config.veda.show_callpath.value) {
//      path_list = vftr_ranklogfile_veda_table_callpath_list(nrows,
//                                                       selected_stacks,
//                                                       stacktree);
