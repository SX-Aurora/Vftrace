...
void vftr_veda_callback_launch_host_enter(VEDAprofiler_data* data) {
   ...
   const char *launchname = "vedaLaunchHostFunc";
   int launchnamelen = strlen(launchname);
   const char *hostname = LaunchKernelData->kernel;
   ...
   // Obtain stack and thread ID for the launched host-kernel
...
void vftr_veda_callback_launch_host_exit(VEDAprofiler_data* data) {
   long long tstart_callback = vftr_get_runtime_nsec();
   long long host_end_time = tstart_callback;

   // get back user data
   user_data_t *user_data = (user_data_t*) data->user_data;
   long long runtime_usec = host_end_time - user_data->start_time;
