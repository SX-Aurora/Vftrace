...
int vftr_MPI_Rget(void *origin_addr, int origin_count,
   ...
   if (vftrace_Pcontrol_level == 0) {
      ...
      vftr_register_request(recv, 1, &origin_count, &origin_datatype,
...
int vftr_MPI_Put(const void *origin_addr, int origin_count,
   ...
   if (vftrace_Pcontrol_level == 0) {
      ...
      long long tstart = vftr_get_runtime_usec();
      int retVal = PMPI_Put(origin_addr, origin_count, origin_datatype,
                            target_rank, target_disp, target_count,
                            target_datatype, win);
      long long tend = vftr_get_runtime_usec();
...
int vftr_MPI_Rput(const void *origin_addr, int origin_count,
   ...
   if (vftrace_Pcontrol_level == 0) {
      ...
      vftr_register_request(send, 1, &origin_count, &origin_datatype,
                            &global_rank, -1, MPI_COMM_WORLD, *request, tstart);
      return retVal;
   }
}
