...
int main (int argc, char *argv[]) {
   ...
   double sum, diff;
   vftr_set_papi_calculator_counters (&calc, values1);  
   sum = vftr_papi_calculator_evaluate (calc, 0);
   diff = vftr_papi_calculator_evaluate (calc, 1);

   printf ("1 + 1 = %.0f\n", sum);
   printf ("1 - 1 = %.0f\n", diff);

   long long values2[2] = {-1, 1}; 
   vftr_set_papi_calculator_counters (&calc, values2);  
   sum = vftr_papi_calculator_evaluate (calc, 0);
   diff = vftr_papi_calculator_evaluate (calc, 1);
