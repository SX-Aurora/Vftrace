...
int main(int argc, char** argv) {
   ...
   if (argc < 2) {
      printf("./gatherv_intercom <msgsize in ints>\n");
      return 1;
   }
   ...
   if (color == 0) {
      ...
      if (my_sub_rank == rootrank) {
         // Receiving rank in receiving subgroup
         root = MPI_ROOT;
         recvcounts = (int*) malloc(sub_comm_remote_size*sizeof(int));
         displs = (int*) malloc(sub_comm_remote_size*sizeof(int));
         int ntot = 0;
         for (int i=0; i<sub_comm_remote_size; i++) {
            recvcounts[i] = nints + sub_comm_size + i;
            displs[i] = ntot;
            ntot += recvcounts[i];
         }
   ...
   MPI_Igatherv(sbuffer, nints, MPI_INT,
                ...
                root, int_comm, &myrequest);
   if (my_rank == rootrank) {
      printf("Gathering message on global rank %d\n", my_rank);
   ...
   if (my_rank == rootrank) {
      for (int irank=0; irank<sub_comm_remote_size; irank++) {
         ...
         for (int i=0; i<recvcounts[irank]; i++) {
            if (rbuffer[i+displs[irank]] != jrank) {
