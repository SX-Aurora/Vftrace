...
int main(int argc, char** argv) {
   ...
   if (argc < 2) {
      printf("./ireduce_scatter_block_intercom <msgsize in ints>\n");
      return 1;
   }
   ...
   int recvcount = count /sub_comm_size;
   int *sbuffer = (int*) malloc(count*sizeof(int));
   for (int i=0; i<count; i++) {sbuffer[i]=my_rank;}
   int *rbuffer = (int*) malloc(recvcount*sizeof(int));
   for (int i=0; i<recvcount; i++) {
      rbuffer[i] = -1;
   }

   MPI_Request myrequest;
   MPI_Ireduce_scatter_block(sbuffer, rbuffer, recvcount, MPI_INT,
                             MPI_SUM, int_comm, &myrequest);
