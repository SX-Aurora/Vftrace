...
int main(int argc, char** argv) {
   ...
   if (argc < 2) {
      printf("./ineighbor_alltoallw_dist_graph <msgsize in ints>\n");
      return 1;
   }
   ...
   MPI_Aint *sdispls = (MPI_Aint*) malloc(outdegree*sizeof(MPI_Aint));
   MPI_Datatype *sendtypes = (MPI_Datatype*) malloc(outdegree*sizeof(MPI_Datatype));
   int nstot = 0;
   ...
   MPI_Aint *rdispls = (MPI_Aint*) malloc(indegree*sizeof(MPI_Aint));
   int nrtot = 0;
   MPI_Datatype *recvtypes = (MPI_Datatype*) malloc(indegree*sizeof(MPI_Datatype));
