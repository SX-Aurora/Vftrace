...
int main(int argc, char** argv) {
   ...
   if (argc < 2) {
      printf("./igather_intercom <msgsize in ints>\n");
      return 1;
   }
   ...
   int *rbuffer = NULL;
   if (my_rank == 0) {
      rbuffer = (int*) malloc(sub_comm_remote_size*nints*sizeof(int));
      for (int i=0; i<sub_comm_remote_size*nints; i++) {
         rbuffer[i] = -1;
      }
   }
   ...
   MPI_Igather(sbuffer, nints, MPI_INT,
