...
int main(int argc, char** argv) {
   ...
   // preparing the intercomm special arrays
   int *recvcounts = (int*) malloc(sub_comm_remote_size*sizeof(int));
   int *displs = (int*) malloc(sub_comm_remote_size*sizeof(int));
   int ntot = 0;
   for (int irank=0; irank<sub_comm_remote_size; irank++) {
      int jrank = minpeerrank + irank;
      recvcounts[irank] = nints - my_rank + jrank;
      displs[irank] = ntot;
      ntot += recvcounts[irank];
   }
