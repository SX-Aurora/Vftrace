PROGRAM alltoallv_inplace
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./alltoallv_inplace <msgsize in integers>"
      STOP 1
   END IF
   ...
   ! Messaging
   CALL MPI_Alltoallv(MPI_IN_PLACE, scounts, sdispls, MPI_DATATYPE_NULL, &
