PROGRAM ireduce_scatter_block_intercom
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./ireduce_scatter_block_intercom <msgsize in integers>"
      STOP 1
   END IF
   ...
   sbuffer(:) = my_rank
   ALLOCATE(rbuffer(recvcount))
   rbuffer(:) = -1

   CALL MPI_Ireduce_scatter_block(sbuffer, rbuffer, recvcount, MPI_INTEGER, &
...
END PROGRAM ireduce_scatter_block_intercom
