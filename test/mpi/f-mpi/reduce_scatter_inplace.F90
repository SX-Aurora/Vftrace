PROGRAM reduce_scatter_inplace
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./reduce_scatter_inplace <msgsize in integers>"
      STOP 1
   END IF
   ...
   ! Message cycle
   CALL MPI_Reduce_scatter(MPI_IN_PLACE, rbuffer, recvcounts, MPI_INTEGER, &
...
END PROGRAM reduce_scatter_inplace
