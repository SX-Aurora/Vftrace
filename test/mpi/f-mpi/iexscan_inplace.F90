PROGRAM iexscan_inplace
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./iexscan_inplace <msgsize in integers>"
      STOP 1
   END IF
   ...
   ! Message cycle
   CALL MPI_Iexscan(MPI_IN_PLACE, rbuffer, nints, MPI_INTEGER, &
