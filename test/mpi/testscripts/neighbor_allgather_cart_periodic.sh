...
vftr_binary=neighbor_allgather_cart_periodic
nprocs=4
ntrials=1
...
do
   ...
   neighborlist[0]="2 2 1 1 0 0"
   neighborlist[1]="3 3 0 0 1 1"
   neighborlist[2]="0 0 3 3 2 2"
   neighborlist[3]="1 1 2 2 3 3"
   ...
   do
      ...
      do
         if [[ "${jrank}" -gt "0" ]] ; then
            ipeer=$(bc <<< "${jrank} + 1")
            # Validate sending
            # check if every message appears twice due to the periodicity
