PROGRAM send_init
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 2) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./send_init <msgsize in integers> <nrepetitions>"
      STOP 1
   END IF
   ...
   IF (my_rank == 0) THEN
      ...
      DO recvrank = 1, comm_size-1
         CALL MPI_Send_init(sbuffer, nints, MPI_INTEGER, recvrank, 0, MPI_COMM_WORLD, &
