PROGRAM accumulate
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./accumulate <msgsize in integers>"
      STOP 1
   END IF
   ...
   IF (my_rank == 0) THEN
      ...
      DO targetrank = 1, comm_size - 1
         ...
         CALL MPI_Accumulate(srbuffer, nints, MPI_INTEGER, targetrank, targetdisp, &
                             nints, MPI_INTEGER, MPI_SUM, window, ierr)
      END DO
   END IF
