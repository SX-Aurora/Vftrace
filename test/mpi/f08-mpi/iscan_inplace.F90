PROGRAM iscan_inplace
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./iscan_inplace <msgsize in integers>"
      STOP 1
   END IF
   ...
   ! Message cycle
   CALL MPI_Iscan(MPI_IN_PLACE, rbuffer, nints, MPI_INTEGER, &
