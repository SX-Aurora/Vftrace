PROGRAM neighbor_allgatherv_dist_graph
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./neighbor_allgatherv_dist_graph <msgsize in integers>"
      STOP 1
   END IF
   ...
   ALLOCATE(displs(indegree))
   ntot = 0
   DO ineighbor = 0, indegree - 1
      IF (inneighbors(ineighbor+1) /= -1)  THEN
   ...
   ! Message cycle
   CALL MPI_Neighbor_allgatherv(sbuffer, nints, MPI_INTEGER, &
...
END PROGRAM neighbor_allgatherv_dist_graph
