PROGRAM testsome
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./testsome <msgsize in intergers>"
      STOP 1
   END IF
   ...
   IF (my_rank == 0) THEN
      ...
      DO WHILE (outcount == 0)
         CALL MPI_Testsome(comm_size-1, requests, outcount, idxs, statuses, ierr)
         IF (outcount == 0) THEN
            WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") &
               "No sending request is completed"
            CALL SLEEP(1)
         ELSE
            DO iidx = 1, outcount
