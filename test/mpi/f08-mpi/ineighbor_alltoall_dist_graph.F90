PROGRAM ineighbor_alltoall_dist_graph
   ...
   IF (COMMAND_ARGUMENT_COUNT() < 1) THEN
      WRITE(UNIT=OUTPUT_UNIT, FMT="(A)") "./ineighbor_alltoall_dist_graph <msgsize in integers>"
      STOP 1
   END IF
   ...
   ALLOCATE(sbuffer(nints,outdegree))
   sbuffer(:,:) = my_rank
   ALLOCATE(rbuffer(nints,indegree))
   rbuffer(:,:) = -1

   ! Message cycle
   CALL MPI_Ineighbor_alltoall(sbuffer, nints, MPI_INTEGER, &
...
END PROGRAM ineighbor_alltoall_dist_graph
