...
bool double_list_sorted(int n, double *list, ascending) {
   bool sorted = true;
   if (ascending) {
      for (int i=1; i<n; i++) {
...
int main(int argc, char **argv) {
   ...
   if (argc < 3) {
      printf("./sort_perm_double <listsize> <ascending>\n");
      return 1;
   }
   ...
   int *perm = NULL;
   vftr_sort_perm_double(n, list, &perm, ascending);
   ...
   bool sorted_other_list = double_list_sorted(n, list2, ascending);
