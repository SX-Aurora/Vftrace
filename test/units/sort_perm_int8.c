...
int main(int argc, char **argv) {
   ...
   if (argc < 3) {
      printf("./sort_perm_int8 <listsize> <ascending>\n");
      return 1;
   }
   ...
   int8_t *list2 = (int8_t*) malloc(n*sizeof(int8_t));
   ...
   int *perm = NULL;
   vftr_sort_perm_int8(n, list, &perm, ascending);

   bool sorted_after = int8_list_sorted(n, list, ascending);
   printf("sorted after: %s\n", sorted_after ? "true" : "false");

   vftr_apply_perm_int8(n, list2, perm);

   bool sorted_other_list = int8_list_sorted(n, list2, ascending);
