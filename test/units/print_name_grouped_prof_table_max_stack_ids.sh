...
set -x
test_name=print_name_grouped_prof_table_max_stack_ids
configfile=${test_name}.json
output_file=${test_name}.out
ref_file=${srcdir}/ref_output/${test_name}.out

# create logfile
echo "{\"name_grouped_profile_table\": {\"max_stack_ids\": 2, \"sort_table\": {\"column\": \"time_excl\", \"ascending\": false}}}" > ${configfile}
