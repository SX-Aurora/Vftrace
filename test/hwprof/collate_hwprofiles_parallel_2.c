...
stacktree_t vftr_init_dummy_stacktree (uint64_t t_call) {
   stacktree_t dummy_stacktree = vftr_new_stacktree();
   char *rootname = "init";
...
int get_stack_idx (stacktree_t *stacktree, uint64_t hash) {
   if (hash == 0) return 0;
...
void vftr_register_dummy_call_stack (stacktree_t *dummy_stacktree, char *stackstring, uint64_t t_call) {
   int slen = strlen(stackstring);
   uint64_t new_hash = vftr_jenkins_murmur_64_hash (slen, (uint8_t*)stackstring); 
   int this_stack_idx = get_stack_idx (dummy_stacktree, new_hash);
   ...
   if (!stack_already_present) {
      ...
      int stack_idx_orig = get_stack_idx (dummy_stacktree, hash_bottom);
...
void vftr_register_dummy_hwprof_stack (stacktree_t *stacktree, char *stackstring, long long *counters) {
   int slen = strlen(stackstring);
   uint64_t new_hash = vftr_jenkins_murmur_64_hash (slen, (uint8_t*)stackstring); 
   int this_stack_idx = get_stack_idx (stacktree, new_hash);
   ...
   if (!stack_already_registered) {
      ...
      uint64_t hash_bottom = vftr_jenkins_murmur_64_hash (strlen(stack_bottom), (uint8_t*)stack_bottom);
      int stack_idx_orig = get_stack_idx (stacktree, hash_bottom);
...
void vftr_update_dummy_observables (stacktree_t *stacktree) {
   int nstacks = stacktree->nstacks;
   vftr_stack_t *stacks = stacktree->stacks; 
   for (int istack = 1; istack < nstacks; istack++) {
      ...
      for (int iprof = 0; iprof < this_stack->profiling.nprofiles; iprof++) {
         ...
         hwprof->observables[0] = 2 * (double)(hwprof->counters_excl[0]);
      }
   }
}
...
int main (int argc, char **argv) {
   ...
   vftrace.hwprof_state.observables = (vftr_observable_t*)malloc (n_observables * sizeof(vftr_observable_t));
   vftrace.hwprof_state.observables[0].name = "dummy_obs";
   vftrace.hwprof_state.observables[0].unit = "dummy_unit";

   stacktree_t dummy_stacktree = vftr_init_dummy_stacktree (10);
    
   vftr_register_dummy_call_stack (&dummy_stacktree, "func0<init", 1);
   vftr_register_dummy_call_stack (&dummy_stacktree, "hwfunc1<init", 2);
   long long c1[] = {1000};
   vftr_register_dummy_hwprof_stack (&dummy_stacktree, "hwfunc1<init", c1);
   long long c2[] = {1500};
   vftr_register_dummy_hwprof_stack (&dummy_stacktree, "hwfunc2<func0<init", c2);

   vftr_update_stacks_exclusive_time (&dummy_stacktree);
   vftr_update_stacks_exclusive_counters (&dummy_stacktree);
   vftr_update_dummy_observables (&dummy_stacktree);

   collated_stacktree_t collated_stacktree = vftr_collate_stacks(&dummy_stacktree);
   vftr_collate_profiles (&collated_stacktree, &dummy_stacktree);

   for (int i = 0; i < nranks; i++) {
      if (myrank == i) {
        fprintf (stdout, "Ranklogfile for rank %d: \n", i);
        vftr_write_ranklogfile_hwprof_obs_table(stdout, dummy_stacktree, vftrace.config);
      }
      fflush(stdout);
   ...
   if (myrank == 0) {
     ...
     vftr_write_hwprof_observables_table (stdout, collated_stacktree, vftrace.config);
   }
